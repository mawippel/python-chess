    ...
    def move(self, dest_square):
        ...
        if move in legal_moves:
            ...
            if self.board.is_checkmate():
                self.label_status['text'] = "Checkmate. (Chequemate)"
            elif self.board.is_stalemate():
                self.label_status['text'] = "It was a draw. (Empate)"
            else:
                self.label_status[
                    'text'] = "Computer's turn. The computer is thinking... (Computador pensando...)"

                self.root.after(100, self.parent.computer_play)
        else:
            self.label_status['text'] = "Fatal Error"
